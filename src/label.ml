...
module type LabelConvention = sig
  type s =
  {
    set: Lexing.position -> section_kind -> ident * ident option -> t -> t;
    set_rule_id: ident option -> t -> t;
    (* valid_rule_label: Lexing.position -> t -> unit; *) (* should not depend on any convention *)
    qualified_name_of_law: label_levels -> ident; (* used in modules.ml *)
    (* qualified_filters_of_law: ('a * 'b) list -> (section_kind * 'a) list; *) (* only used internally *)
    qualified_name_of_level: label_levels -> ident; (* TODO: should this be "exported" or is this an implementation detail? *)
    qualified_name_of_level_simple: label_levels -> ident; (* TODO: should this be "exported" or is this an implementation detail? *)
    (* qualified_filters_of_level: (int -> 'a) -> ('b * 'c) list -> ('a * 'b) list; *)
    qualified_name_of_article: label_levels -> ident; (* TODO: should this be "exported" or is this an implementation detail? *)
    string_of_rule_id: ident option -> ident;
    qualified_name: t -> ident;
    qualified_id: t -> ident;
    qualified_filters: t -> (section_kind * ident) list;
    full_filters: t -> (section_kind * ident) list;
    (* scope: t -> t; *) (* should likely be consistent across different conventions *)
    (* prefixes: t -> t list; *) (* should likely be consistent across different conventions *)
    combine_prefix_with_exception_ident: ident -> t -> ident;
    get_full_name: ident -> t -> Lexing.position -> (ident, t, Base.String.comparator_witness) Base.Map.t -> ident
  }
...
(** error handling, namely of invalid sublevel indexes, is delegated to the 
    naming convention module, where the appropriate qualified name can be
    produced for the error message *)
...
module StandardConvention : LabelConvention = struct
  type s =
  {
    set: Lexing.position -> section_kind -> ident * ident option -> t -> t;
    set_rule_id: ident option -> t -> t;
  ...
  let set pos section_kind label l = try set section_kind label l with
    | Invalid_argument _ -> Util.label_error ("wrong sub-level index (" ^ string_of_section_kind section_kind ^ ", " ^ qualified_name l ^ ")") pos
  
  let set_rule_id = set_rule_id

  let convention =
  {
    set = set;
    set_rule_id = set_rule_id;
    qualified_name_of_law = qualified_name_of_law;
    qualified_name_of_level = qualified_name_of_level;
    qualified_name_of_level_simple = qualified_name_of_level_simple;
    qualified_name_of_article = qualified_name_of_article;
    string_of_rule_id = string_of_rule_id;
    qualified_name = qualified_name;
    qualified_id = qualified_id;
    qualified_filters = qualified_filters;
    full_filters = full_filters;
    combine_prefix_with_exception_ident = combine_prefix_with_exception_ident;
