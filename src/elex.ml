...
type erule_compilation =
  | ECImplication   of int * erule_type * (Lexing.position * Eformula.t) list * (Lexing.position * Eformula.t) list * (Lexing.position * Eformula.t) list * epattern * (Lexing.position * Eformula.t) list * epattern * rule_type * rule_constr list
  | ECDefinition    of int * erule_type * (Lexing.position * Eformula.t) list * (Lexing.position * Eformula.t) list * (Lexing.position * Eformula.t) list * epattern * (Lexing.position * Label.t * Lex.reference) list * Eformula.t
...
let get_obligation_params compilation_rules = function
  | EObligation c_idx ->
    (match Map.find_exn compilation_rules c_idx with
      | ECImplication (_, _, f, _, _, p, g, q, rt, rcs) -> (f, p, g, q, rt, rcs)
      | _ -> assert false)
  | _ -> assert false
...
let get_exception_params compilation_rules = function
  | EException c_idx ->
    (match Map.find_exn compilation_rules c_idx with
      | ECDefinition (_, _, f, _, _, p, erefs, _) -> (f, p, erefs)
      | _ -> assert false)
  | _ -> assert false
