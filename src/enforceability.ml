...
(** map, containing information whether or not a paramter of an internal event is
    past-guarded or not *)
type pg_map = (string, bool list, String.comparator_witness) Map.t
let string_of_pg_map pg_map =
  if Map.is_empty pg_map then "pg_map is empty"
  else
  Map.map pg_map ~f:(fun l -> List.map l ~f:Bool.to_string |> Util.string_of_string_list)
  |> Map.to_alist
...
let rec is_past_guarded ?(pg_map: pg_map=Map.empty (module String)) s x p f =
  ...
  | TPredicate (e, ts, _) ->
    begin match Map.find s.tevents e with
      | Some (_, _, TItl, _) ->
        let pg_event_params = Map.find_exn pg_map e in
        List.exists2_exn ts pg_event_params ~f:(fun t pg -> pg && List.mem (TTerm.fv_list [t] |> List.map ~f:fst) x ~equal:String.equal)
  ...
  | TIff (_, f, g) when p -> is_past_guarded s x (not p) f && is_past_guarded s x p g
                               || is_past_guarded s x p f && is_past_guarded s x (not p) g
  | TIff (_, f, g) -> (is_past_guarded s x (not p) f || is_past_guarded s x p g)
                        && (is_past_guarded s x p f || is_past_guarded s x (not p) g)
  | TExists (y, f)
...
module Errors = struct
  ...
  type error =
    ...
    | EPattern of LexingInfo.t * string * tpformula * EnfType.t
    | ERule of LexingInfo.t * string
    ...
    begin match e with
      ...
      | EFormulasTransparent (None, fs, t) ->
        Printf.sprintf
          ...
          (EnfType.to_string t)
          (LexingInfo.union_all (List.map fs ~f:(fun f -> f.pos)) |> LexingInfo.to_string)
...
module Constraints = struct
  ...
  let verdict_to_string v = match v with
    | Possible c -> "Possible: " ^ to_string c
    | Impossible e -> "Impossible: " ^ Errors.to_string e
...
let rec types s pols (t: EnfType.t) (f: Tformula.t) =
  ...
  | Cau -> begin
      ...
      | TIff (_, f, g) -> conj (disj (types Sup f) (types Cau g))
  ...
  | Sup -> begin
      ...
      | TIff (_, f, g) -> disj (conj (types Cau f) (types Sup g))
...
let rec convert s (pols: ('a, 'b, 'c) Base.Map.t) b enftype (form: Tformula.t) : Eformula.t option =
  ...
  match f with Some f -> Some Eformula.{ f; variable_instantiations=[]; enftype; id = 0; pos = form.pos } | None -> None
  ...
  let aux = function
    ...
    | TCDefinitionDis (disjuncts, {f=Tformula.TPredicate (name, _, _); _}) ->
      ...
      (match Map.find events name with
        | Some (_, _, Lex.TItl, _) -> ()
        | Some (_, _, pol, _) ->
          let err_msg = Printf.sprintf "Can only constitute 'Internal' events, but \"%s\" is \"%s\"\nat locations:\n%s" name (Lex.string_of_pol pol) (List.map positions ~f:LexingInfo.to_string |> Util.string_of_string_list_new_line) in
  ...
  let rec aux visited rest =
    match rest with
      ...
      | _ ->
        ...
        | [] -> Err.enf_error "Circular dependency of internal events" None
        | _ -> ()
        end;
...
let type_tpformula itl_srp (s:Tlex.tprog) (pos: LexingInfo.t) pols enftype (tpf: tpformula) : verdict =
  let types_fun = get_types_fun itl_srp in
  match enftype with
  | Cau ->
...
let get_exception_predicates ?(negated=false) (s: Tlex.tprog) rule_idx =
  ...
  if negated then
    List.map exception_predicates ~f:(fun x -> Tformula.tneg [] x.pos x)
...
let check_pol_constrs pos pols pol_constrs =
...
let type_exceptions itl_srp s pols (exceptions: Tformula.t list) =
  let exceptions_neg = List.map exceptions ~f:(fun f -> Tformula.tneg [] f.pos f) in
...
let parse_rule_constraints pos pols n rcs =
    ...
    let pols = match pol_constr with
      | Some pols' ->
        check_pol_constrs pos pols pols'
...
let is_past_guarded_tcrule_exn ?(pg_map: pg_map=Map.empty (module String)) s rule var positions =
  ...
  | TCImplication (_, _, pos, pf1, ex, sc, pf2, _, _) ->
     let ex_neg = List.map ex ~f:(fun f -> Tformula.tneg [] f.pos f) in
     if not (
       ...
       let err_msg =
         Printf.sprintf
           ...
           var
           (LexingInfo.to_string positions)
       in
       Err.enf_error err_msg (Some pos)
  ...
  | TCDefinitionDis (disjuncts, _) ->
     let aux (disjunct: tdisjunct) =
       let ex_neg = List.map disjunct.exceptions ~f:(fun f -> Tformula.tneg [] f.pos f) in
     ...
     if not (Map.for_all disjuncts ~f:aux) then
       let err_msg =
         Printf.sprintf
           ...
           (LexingInfo.to_string positions)
       in
       let pos = LexingInfo.union_all (List.map (Map.data disjuncts) ~f:(fun d -> d.def_positions)) in
       Err.enf_error err_msg (Some pos)
...
let fv_of_tcrule = function
  | TCImplication (_, _, _, pf1, ex, sc, pf2, _, _) ->
    let free_vars = fv_of_tpformulas pf1 in
    let free_vars = fv_of_tpformulas ~map:free_vars pf2 in
    let free_vars = fv_of_tformulas ~map:free_vars ex in
    let free_vars = fv_of_tformulas ~map:free_vars sc in
    free_vars
  | TCDefinition (_, _, _, pf, ex, sc, _, _) ->
    let free_vars = fv_of_tpformulas pf in
    let free_vars = fv_of_tformulas ~map:free_vars ex in
    let free_vars = fv_of_tformulas ~map:free_vars sc in
    free_vars
  | TCDefinitionDis (disjuncts, _) ->
    ...
    let free_vars = Map.fold disjuncts ~init:free_vars ~f:(fun ~key:_ ~data:disjunct free_vars ->
      let free_vars = fv_of_tpformulas ~map:free_vars disjunct.pf in
      let free_vars = fv_of_tformulas ~map:free_vars disjunct.exceptions in
      let free_vars = fv_of_tformulas ~map:free_vars disjunct.scopes in
      free_vars
    ) in
    free_vars
...
let vars_are_past_guarded_tcrule_exn ?(pg_map = Map.empty (module String)) s vars rule =
  ...
  let aux ~key:var ~data:positions =
    is_past_guarded_tcrule_exn ~pg_map s rule var positions
  in
...
let tformula_term_equalities (ts1: TTerm.t list) (ts2: TTerm.t list) =
  List.map2_exn ts1 ts2 ~f:(fun t1 t2 -> Tformula.teqconst [] t2.pos t1 t2)

let get_trm_name (t: TTerm.t) = match t.trm with
  | TTerm.TVar x -> x
  | _ -> assert false

let type_tdisjunct s itl_srp pols pg_map t rule (d: tdisjunct) =
  let v_ex = type_exceptions itl_srp s pols d.exceptions in
  let v_sc = type_scopes itl_srp s pols d.scopes in
  let v_pf = type_tpformula itl_srp s d.rule_pos pols t d.pf in
  ...
  begin match t with
    ...
    | Sup -> (* only one part of the definition must be Sup *)
      ...
      let _, fv_unbound = Map.partitioni_tf (fv_of_tcrule rule) ~f:(fun ~key:x ~data:_ -> List.mem param_names x ~equal:String.equal) in
      (* let pg_map = update_pg_map s pg_map e fv_params rule in *)
      vars_are_past_guarded_tcrule_exn ~pg_map s fv_unbound rule;
...
let type_tcrule itl_srp (s:Tlex.tprog) (pg_map: pg_map) (verdict: verdict) rule: verdict =
  ...
  match rule with
    ...
    | TCDefinition (idx, _, _, pf1, ex, sc, _, f2) ->
      ...
      let _, fv_unbound = Map.partitioni_tf fv ~f:(fun ~key:x ~data:_ -> List.mem param_names x ~equal:String.equal) in
      (* let pg_map = update_pg_map s pg_map e fv_params rule in *)
    ...
    | TCDefinitionDis (disjuncts, g) ->
      ...
      let aux d =
        ...
        let pols_tr = update_pols_with_transparency_conditions pols v_pols in
        let type_tdisjunct = type_tdisjunct s itl_srp pols_tr pg_map t rule in
        Map.map disjuncts ~f:type_tdisjunct
        |> Map.data |> Constraints.disj' |> conj verdict
      in
      let verdicts = List.map dnf_of_v ~f:aux in
      Constraints.disj' verdicts
    ...
    List.iter used ~f:(fun name ->
      if not (List.mem all_defined_events name ~equal:String.equal) then
        let warning = Printf.sprintf
            ...
            name in
        Err.warning warning (Some pos)) in
...
let pg_map_of_tcrule tprog pg_map: tcrule -> pg_map = function
  | TCImplication _ -> pg_map
  | TCDefinition (_, _, _, tpf, ex, sc, _, g) ->
    let e = get_predicate_name_exn g in
    let args = get_predicate_params_exn g in
    let arg_names = List.map args ~f:get_trm_name_exn in
    let pg_in_tpf = List.map arg_names ~f:(fun x -> is_past_guarded_tpformula ~pg_map tprog x true tpf) in
    let pg_in_ex = List.map arg_names ~f:(fun x -> is_past_guarded_tformulas ~pg_map tprog x false ex) in
    let pg_in_sc = List.map arg_names ~f:(fun x -> is_past_guarded_tformulas ~pg_map tprog x true sc) in
    let pg_in_def = List.map3_exn pg_in_tpf pg_in_ex pg_in_sc ~f:(fun a b c -> a || b || c) in
    Map.add_exn pg_map ~key:e ~data:pg_in_def
  | TCDefinitionDis (tdisjuncts, g) ->
    let e = get_predicate_name_exn g in
    let pg_disjunct_params = Map.map tdisjuncts ~f:(fun d ->
      let fv_args = List.map d.params_original ~f:(fun t -> TTerm.fv_list [t] |> List.map ~f:fst) in
      let aux v = is_past_guarded_tpformula ~pg_map tprog v true d.pf ||
                  is_past_guarded_tformulas ~pg_map tprog v false d.exceptions ||
                  is_past_guarded_tformulas ~pg_map tprog v true d.scopes
      in
      List.map fv_args ~f:(fun vars -> List.for_all vars ~f:aux)
    ) in
    let pg_disjuncts =
      pg_disjunct_params
      |> Map.data
      |> List.fold
        ~init:(List.map (get_predicate_params_exn g) ~f:(fun _ -> true))
        ~f:(fun acc pg_disjunct -> List.map2_exn acc pg_disjunct ~f:(fun a b -> a && b))
    in
    Map.add_exn pg_map ~key:e ~data:pg_disjuncts

let pg_map_of_tcrules tprog tcrules: pg_map =
  List.fold tcrules ~init:(Map.empty (module String)) ~f:(pg_map_of_tcrule tprog)

let type_tcrules (tprog:Tlex.tprog) (tcrules: (int, tcrule, 'a) Map.t) : (string, EnfType.t * bool, 'string_comp) Map.t * int list * 'itl_srp * pg_map =
  ...
  let pg_map = pg_map_of_tcrules tprog (List.rev tcrules_sorted) in
  let verdict = List.fold tcrules_sorted ~f:(type_tcrule (itl_itvs, itl_strict) tprog pg_map) ~init:(Possible CTT) in
  ...
  in
  found_pol_constraints, sorted_rule_indices, (itl_itvs, itl_strict), pg_map
...
let combine_constitutive_rules rules: tcrule list * 'params_map =
  let collect_and_separate_event_definitions (params_map, m) (pos, idx, rule, exceptions, scopes) =
    let trule_type = match rule with
      | TConstitutive _ -> TRTConstitutive
      | TExceptionC _ -> TRTExceptionC
      | _ -> assert false
    in
    let separate_event_definitions pf ((params_map, m'): 'params_map * (string, 'a, 'b) Map.t) f = match f.f with
      | Tformula.TPredicate (name, terms, _) ->
        let fvs = fv_of_tpformula ~map:(fv_of_tformulas ~map:(fv f) (exceptions @ scopes)) pf in
        let params_new = match Map.find params_map name with
              | Some params -> params
              | None -> List.map terms ~f:(fun _ -> let p = fresh_param () in TTerm.{ trm = (TVar p); tt = TypeVar p; pos = LexingInfo.dummy });
        in
        ...
        let disjunct: tdisjunct =
          { rule_id = idx;
            ...
            scopes = scopes;
            fv_renaming = Map.map fvs ~f:(fun _ -> fresh_free_var ());
    ...
    | _ -> assert false
  in
  let params_map, event_def_map = List.fold rules ~init:((Map.empty (module String)), (Map.empty (module String))) ~f:collect_and_separate_event_definitions in
...
let convert_enforceable_exn s pols b enftype f =
  match convert s (Map.map pols ~f:fst) b enftype f with
  | Some f' -> f'
  | None -> 
...
let convert_enforceable_tformulas ?(formulas_are_disjunction=false) (s: tprog) (enftype: EnfType.t) pols b (fs: Tformula.t list) : Eformula.t list * int option =
  ...
  | Cau, false | Sup, true -> List.map fs ~f:(convert enftype), None
  | Sup, false | Cau, true -> 
    let find_first_possible _ f =
      match types s pols enftype f with
      | Possible _ -> true
      | Impossible _ -> false
    in
...
let convert_enforceable_tdisjunct itl_srp s pols b t (td: tdisjunct): edisjunct * enf_ecdefinition option =
  match t with
  | Cau ->
    let v_ex = type_exceptions itl_srp s pols td.exceptions in
    let v_sc = type_scopes itl_srp s pols td.scopes in
    let v_pf = type_tpformula itl_srp s td.rule_pos pols t td.pf in
    ...
    in (* TODO: is it actually necessary to compute a 'new' verdict here ?
                - in TCImplication and the Sup case for TCDefinition, it is 
                  ...
                  directly necessary *)
    let ex', _ = convert_enforceable_tformulas ~formulas_are_disjunction:true s Sup pols b td.exceptions in
    let sc', _ = convert_enforceable_tformulas s Cau pols b td.scopes in
    let epf, constr_opt = convert_enforceable_tpformula s Cau pols b td.pf in
  ...
  | Sup ->
    begin match type_exceptions itl_srp s pols td.exceptions with
      | Possible _ ->
        let epf = epformula_of_tpformula s.tevents td.pf in
        let ex', i_opt = convert_enforceable_tformulas ~formulas_are_disjunction:true s Cau pols b td.exceptions in
        let sc' = List.map td.scopes ~f:(Eformula.of_tformula pols) in
      ...
      | _ -> begin match type_scopes itl_srp s pols td.scopes with
        | Possible _ ->
          let epf = epformula_of_tpformula s.tevents td.pf in
          let ex' = List.map td.exceptions ~f:(Eformula.of_tformula pols) in
          let sc', i_opt = convert_enforceable_tformulas s Cau pols b td.scopes in
        ...
        | _ ->
          begin match type_tpformula itl_srp s td.def_positions pols Sup td.pf with
          | Possible _ ->
            let epf, constr_opt = convert_enforceable_tpformula s Cau pols b td.pf in
            let ex' = List.map td.exceptions ~f:(Eformula.of_tformula pols) in
            let sc' = List.map td.scopes ~f:(Eformula.of_tformula pols) in
  ...
  | Obs -> edisjunct_of_tdisjunct s.tevents td, None
  | _ -> assert false

let convert_tcrule pg_map itl_srp (s: tprog) (pols: (string, (EnfType.t * bool), 'string_comp) Map.t) b = function
              ...
              | Possible _ ->
                ...
                let pf1_used, f1s_unused = 
                  if suppress_conditions then pf1, []
                    ...
                    pf1_used, fs1_unused
                in
                let epf1_used, constr_opt = convert_enforceable_tpformula s Sup pols b pf1_used in
        ...
        begin match t with
          | Cau ->
            ...
            let edisjuncts_and_constrs = Map.map tdisjuncts
              ~f:(convert_enforceable_tdisjunct itl_srp s pols b Cau) in
          ...
          | Sup ->
            let type_tdisjunct = type_tdisjunct s itl_srp pols pg_map t rule in
            ...
            let aux2 ~key ~data:td =
              if key = first_possible then
                ...
                let ed = edisjunct_of_tdisjunct s.tevents td in
                ed, None
            in
            ...
            let edisjuncts = Map.map edisjuncts_and_constrs ~f:fst in
            ECDefinitionDis (edisjuncts, eg, constr)
          | _ -> assert false
        end
      | None ->
        let edisjuncts = Map.map tdisjuncts ~f:(edisjunct_of_tdisjunct s.tevents) in
        ECDefinitionDis (edisjuncts, eg, None) 
    end

let convert_tcrules pg_map itl_srp tprog pols b rules =
  Map.map rules ~f:(convert_tcrule pg_map itl_srp tprog pols b)
