...
\begin{align*}
  rule\_verb ::=&~\kw{oblige} \mid \kw{permit} \mid \kw{constitute} \mid \kw{except}\\
  rule\_decl ::=&~\kw{rule}~[string]\\
                &~[\kw{whenever}~[pre\_opt]\\
                &\qquad [label_{11}\kw{:}]~expr_{11}\\
                &\qquad \dots\\
                &\qquad [label_{1k}\kw{:}]~expr_{1k}]\\
                &~rule\_verb~[post\_opt]\\
                &\qquad [label_{21}\kw{:}]~expr_{21}\\
                &\qquad \dots\\
                &\qquad [label_{2k}\kw{:}]~expr_{2k}
\end{align*}
In the following we review the syntax of expressions $expr_{ij}$ (Section~\ref{sec:expr}), the available verbs
(Section~\ref{sec:expr_verb}) and the temporal patterns determined by $pre\_opt$ and $post\_opt$ (Section~\ref{sec:opt}).

\subsubsection{Temporal patterns.}

\label{sec:opt}

Except for $\kw{except}$ rules, every \Lex{} rule declaration represents a statement of the form
$$\kw{ALWAYS} \left[pre(expr_{11}~\kw{AND}~\ldots~\kw{AND}~expr_{1k})~\kw{IMPLIES}~post(expr_{21}~\kw{AND}~\ldots~\kw{AND}~expr_{2k})\right]$$
where $pre$ and $post$ are determined by $pre\_opt$ and $post\_opt$, respectively. These options use the following
...
Table~\ref{tab:patterns} lists available $(pre\_opt, post\_opt)$ pairs as well as the corresponding $pre$ and $post$ functions.

\begin{table}
  \centering
\begin{tabular}{lllll}
  Pattern & $pre\_opt$ & $post\_opt$ & $pre(\phi)$ & $post(\psi)$ \\
  \hline
          & -- & -- & $\phi$ & $\psi$\\
          & -- & $\kw{eventually}~[time\_interval]$ & $\phi$ & $\kw{EVENTUALLY}_I~\psi$\\
          & -- & $\kw{always}~[time\_interval]$ & $\phi$ & $\kw{ALWAYS}_I~\psi$\\
  TBC....
\end{tabular}\\[10pt]
