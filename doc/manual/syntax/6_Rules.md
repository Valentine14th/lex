...
[<indentation> doc_string]
    ...
    verb 
        ...
        ...
        expression | reference
[[[`transparently`] `enforceable`] [(`suppressing` | `causing`) ident, ..., ident]*]
...
observable event LegalWarrant
    """company {c} is required by public authorities to process data {d}"""
    c : company
    d : data
```
...
```
expr := ident `(` term `,` ... `,` term `)` # Event / predicate
      | term `=` term                       # Equality
      ...
      | ident `<-` agg `(`ident`;` ident`,` ...`,` ident`;` expr`)` # Aggregation
...
untop := `PREVIOUS` | `NEXT` | `ONCE` | `EVENTUALLY` | `HISTORICALLY` | `ALWAYS`
bintop := `SINCE` | `UNTIL`
quant := `EXISTS` | `FORALL`
...
const := `true` | `false` | int | float | string | time | span | money
num := (`0`-`9`)+
int := [-] num
float := [-] num+ `.` num*
string := `"` .* `"`
