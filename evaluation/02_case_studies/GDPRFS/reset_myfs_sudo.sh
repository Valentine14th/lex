...
echo "sudo -E PYTHONPATH=. .gdprfs-venv/bin/python3 gdprfs/myfs.py /tmp/mnt -f -o allow_other" # Use sudo to run the FUSE daemon as root in order to 1)Open /dev/fuse (to register and communicate with the kernel), and 2) Read/write /var/lib/gdprfs/upper and so make a copy in /var/lib/gdprfs/mirror