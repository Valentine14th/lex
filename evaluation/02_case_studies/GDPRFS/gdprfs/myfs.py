...
def replay_from_consent_db(logger):
    ...
    config_path = Path(__file__).resolve().parents[1] / "external_consent_platform" / "event_config.yaml"
...
def events_for_read(path):
    ...
    # Sanitize LibreOffice lock files: .~lock.alice_a.csv# → alice_a.csv
    real_base = base
...
def delete_causation_handler(event_list):
    ...
    args is list[str], e.g. ['bob_b.txt']: not list[list[str]]."""
...
def events_for_read_or_skip(path):
    ...
    # Sanitize lock file name: .~lock.alice_a.csv# → alice_a.csv
    if base.startswith(".~lock.") and base.endswith("#"):
...
class MyFS(Fuse):
    ...
    def open(self, path, flags):
        ...
        p = _upper(path)
        if not p.exists():
            # Kernel may call open() with O_CREAT due to a stale dentry cache entry
            # even after a recent unlink(). Handle this gracefully: if the caller wants
            # to write (O_WRONLY / O_RDWR), create the file so the operation succeeds.
            WRITE_FLAGS = os.O_WRONLY | os.O_RDWR
            if flags & WRITE_FLAGS:
                print(f"[OPEN] File missing in upper but opened with write flags ({flags:#o}); "
                      f"creating: {path}")
                _ensure_parent(p)
                with open(p, "wb"):
                    pass
...
if __name__ == "__main__":
    ...
    def start_consent_poller():
        try:
            subprocess.Popen(
                ["python3", str(Path(__file__).resolve().parents[1] / "external_consent_platform" / "poller.py")]
            )
