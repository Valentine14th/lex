...
├── openai.secret                        # OpenAI API key for LLM Analyzer
...
OPENAI_API_KEY=sk-...
```
Alternatively, you can put your API key in file `openai.secret`.
...
| `alias_person_map` | id, alias (unique), person_id | Human-confirmed name aliases (e.g., "Charli" → "Charlie") |
...
|----------|------|--------|---------|
| 1 | **`.gdprowner`** | Root-only config declaring ownership rules (managed via `sudo`) | `bob_b: team_alpha/**` → all files under `team_alpha/` belong to bob_b |
| 2 | **Folder name** | Folder name matches a known Person | Folder `dave_d/` → matched to Dave D (dave_d) |
| 3 | **Filename** | Filename contains a Person's name | File `bob_b_report.txt` → matched to Bob B |
| 4 | **Content** (fallback) | File content scanned for names | Text contains "Bob B" → linked to bob_b |

- `.gdprowner` is located at `/var/lib/gdprfs/.gdprowner`, one rule per line (e.g. `bob_b: team_alpha/**`).
...
```bash
echo "bob_b: team_alpha/**" | sudo tee -a /var/lib/gdprfs/.gdprowner
```
...
bob_b: team_alpha/**
```
This says: any file under `team_alpha/` belongs to `bob_b`, but "team_alpha" doesn't match any Person name, so tiers 2-4 won't trigger.
...
```bash
[INIT] Loaded gdprowner rules: [('bob_b', 'team_alpha/**')]
```

**3. Create a test file**
1. Create the `/team_alpha` folder in `/tmp/mnt`
2. Create a file named `notes.txt` in the `/team_alpha` folder.
3. Write the file content "some random data", then save the file.


In the FUSE logs you should see:
```bash
[GDPROWNER] Manual override by internal person, in path ...: assigning notes.txt → bob_b (skips folder/filename/content)
```

**4. Verify enforcement works**
Inside an active session:

If `bob_b` has active consent: Then if the internal user opens `/tmp/mnt/team_alpha/notes.txt`, then the "some random data" content is shown.

Then revoke `bob_b`'s consent (via the external consent platform at `:5000`), and: if the internal user reopens `/tmp/mnt/team_alpha/notes.txt`, then `"REDACTED"` is shown instead of the real file content.
