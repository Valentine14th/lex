...
class InstrumentURL(Instrument):
    ...
    def generate_output(self, qualname, response, request, *args, **kwargs):
        ...
        if (qualname, 'output') in self.logger.pep:
            ...
            cau_flag, sup_flag, self.logger.cau_events, self.logger.sup_events = self.send_events(events, qualname)
            if sup_flag:
                response = self.handler_output(self.logger.sup_events, self.logger.pep.sup_event_map, request, response, qualname, *args, **kwargs)
            elif cau_flag:
                response = self.handler_output(self.logger.cau_events, self.logger.pep.cau_event_map, request, response, qualname, *args, **kwargs)
        return response
