...
@InstrumentORM(
	logger,
    {"User.first_name", "User.last_name", "User.email", "User.save", "User.last_login"},
    info=info_user, events={'read', 'write', 'execute'})
...
@InstrumentORM(
	logger,
	{"Like.user", "Like.twit", "Like.created_at"},
    info=info_like, events={'read', 'write'}
)
...
@InstrumentORM(
    logger,
    {"Reply.content", "Reply.created_at", "Reply.author", "Reply.parent"},
    info=info_reply, events={'read', 'write'}
)
...
@InstrumentORM(
    logger,
    {"Repost.original", "Repost.user", "Repost.message", "Repost.created_at"},
    info=info_repost, events={'read', 'write'}
)
...
@InstrumentORM(
    logger,
    {"DirectMessage.sender", "DirectMessage.recipient", "DirectMessage.content", 
	 "DirectMessage.is_read", "DirectMessage.created_at"},
    info=info_dm, events={'read', 'write'}
)
