...
class EnfPal(Enforcer):

    def __init__(self, exe : str, sig : str, formula : str, *args, **kwargs):
        super(EnfPal, self).__init__(*args, **kwargs)
        self.exe     : str = exe
        self.sig     : str = sig
...
"""
def writer(enforcer : Enforcer, log_file : Union[str, None]) -> None:
...
"""
reader thread matches response of enforcer with statments from read_queue;
used to create a proactive thread or wake up worker thread
"""            
def reader(enforcer : Enforcer) -> None:
...
"""
def timer(enforcer : Enforcer, order : PriorityQueue, unit : float = 1):
...
def handle_proactive_commands(msg : str, enforcer : Enforcer):
    start = time()
...
"""
def spawn_proactive_thread(msg : Dict[str, Any], enforcer : Enforcer) -> None:
