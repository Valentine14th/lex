...
class InstrumentURL(Instrument):
    ...
    """
    def generate_url_map(self, urlpatterns, mp, mapping = Mapping()):
    ...
    def generate_input(self, qualname, io, view, request, **kwargs):
        events = []
        if (qualname, 'input') in self.logger.enforcer.mp and (request.method == "GET" or request.method == "POST"):
            x = self.logger.enforcer.mp[(qualname, io)]
    ...
    def generate_output(self, qualname, response, request, *args, **kwargs):
        events = []
        if (qualname, 'output') in self.logger.enforcer.mp:
            ...
            if sup_flag:
                response = self.handler_output(self.sup_events, self.logger.enforcer.mp.sup_event_map, request, response, qualname, *args, **kwargs)
            elif cau_flag:
                response = self.handler_output(self.cau_events, self.logger.enforcer.mp.cau_event_map, request, response, qualname, *args, **kwargs)
        return response
    
    def handler_output(self, events, mp, request, response, qualname, *args, **kwargs):
        max = max_element(self.logger.enforcer.mp.sup_graph, events)
        for el in max:
            handler = mp.get(el) 
