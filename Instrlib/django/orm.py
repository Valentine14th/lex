...
from instrlib.event import Event, Generic
...
class InstrumentORM(Instrument):
    ...
    def __init__(self, logger : Logger, mapping : Set[str], info = None, events : Union[Set[str], None] = None):
        self.target = None
    ...
    """
    def add_filter_check(self, instr, cls):
        qualname = cls.__qualname__
        names_to_check = [t[1] for t in instr.logger.mp if len(t) == 3 and t[0] == qualname and t[2] == 'read']
        @classmethod
        def filter_check(cls, queryset):
            ...
            for name in names_to_check:
                f = instr.logger.mp[(qualname, name, 'read')]
                for obj in objects:
            ...
            if cau_flag:
                instr.invoke_handler(
                    instr.logger.enforcer.mp.cau_graph, 
                    cau_events, 
                    instr.logger.cau_enc, 
                    instr.logger.enforcer.mp.cau_event_map, 
    ...
    def overwrite_getattribute(self, instr, cls):
        def custom_getattribute(self, name):
            ...
            if hasattr(cls, name) and not isinstance(getattr(cls, name), property) and not callable(getattr(cls, name)) and not hasattr(self, '_read_ok'):
                qualname = cls.__qualname__
                if (qualname, name, 'read') in instr.logger.mp:
                    f = instr.logger.mp[(qualname, name, 'read')]
                    ...
                    if self_str is not None:
                        ...
                        if sup_flag:
                            return instr.invoke_handler(
                                instr.logger.enforcer.mp.sup_graph, 
                                sup_events, 
                                instr.logger.sup_enc, 
                                instr.logger.enforcer.mp.sup_event_map, 
                                object.__getattribute__(self, name))
            return object.__getattribute__(self, name)
